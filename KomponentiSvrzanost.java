...
class Graph<E> {
   ...
   int[] najdikomponenti(int start) {
       ...
       Stack<Integer> integeri = new Stack<Integer>();
       ...
       integeri.push(start);
       int n[] = new int[integeri.size()];
       while (!integeri.isEmpty()) {
           n[k++] = integeri.pop();
       }
       return n;
   }
...
public class KomponentiSvrzanost {
   ...
   public static void main(String[] args) throws Exception {
       ...
       for (int i = 0; i < komponenti.length - 1; i++) {
           if (komponenti[i] > komponenti[i + 1]) {
               int tmp = komponenti[i];
               komponenti[i] = komponenti[i + 1];
               komponenti[i + 1] = tmp;
               i = -1;
           }
